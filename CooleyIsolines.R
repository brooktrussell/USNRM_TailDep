		...
		gridSize = 100, asympIndep = F, etaProp = .02, beta = NULL)
{
	...
	out$gpdOut1 <- gpdOut1
	out$gpdOut2 <- gpdOut2
	#out$marg.qtile <- invTransMar(.01, dat, 1, mar1Prop, mar1Width, gpdOut1)#try this to get estimated marginal .99 quantile
	if(asympIndep)
	{
		out$minVec <- minVec
		out$etaHat <- etaHat
				...
				xlim = NULL, ylim = NULL)
{
	...
	plot(out$data, xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, cex = cex, col = col)
...
#create scatterplot with hists on each axis
#from: https://www.r-bloggers.com/2011/06/example-8-41-scatterplot-with-marginal-histograms/
scatterhist = function(x, y, xlab="", ylab="", numbreaks=12, LSline=FALSE, xlabSCL=.8, ylabSCL=.8, extContours=FALSE){
  zones=matrix(c(2,0,1,3), ncol=2, byrow=TRUE)
  layout(zones, widths=c(4/5,1/5), heights=c(1/5,4/5))
  xhist = hist(x, plot=FALSE, breaks=seq(min(x),max(x),length=12))
  yhist = hist(y, plot=FALSE, breaks=seq(min(y),max(y),length=12))
  top = max(c(xhist$counts, yhist$counts))
  par(mar=c(4.3,4.3,1,1))
  plot(c(min(x),max(x)),c(min(y),max(y)),type="n",xlab="",ylab="")
  points(x,y)
  if (LSline==TRUE){abline(lm(y~x))}
  if (extContours==TRUE){
    contoursOut <- xContours(dat = cbind(x,y), projContourLevels = seq(.00001,.000001,length=5), gridSize = 25)
    lines(contoursOut$contourOrig, col = 2)
	for(i in seq(1, length(contoursOut$projContours)))
	{
		lines(contoursOut$projContours[[i]], col = i + 2)
	}
      }
  par(mar=c(0,3,1,1))
  ...
  mtext(xlab, side=1, line=1, outer=TRUE, adj=0, 
    at=xlabSCL * (mean(x) - min(x))/(max(x)-min(x)))
  mtext(ylab, side=2, line=1, outer=TRUE, adj=0, 
    at=(ylabSCL * (mean(y) - min(y))/(max(y) - min(y))))
}
